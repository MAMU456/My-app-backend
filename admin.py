...
def get_all_orders(db: Session = Depends(get_db), admin=Depends(get_current_admin)):
    ...
    for o in orders:
        ...
        result.append({
            ...
            "items": [{"product_name": i.product_name, "quantity": i.quantity, "subtotal": i.subtotal} for i in o.items]
        })
    return result

@router.patch("/orders/{order_id}/status")
def update_order_status(order_id: int, status: str, db: Session = Depends(get_db), admin=Depends(get_current_admin)):
    from models import Order
